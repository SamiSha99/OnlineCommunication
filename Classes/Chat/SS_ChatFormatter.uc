...
static function bool GetLocalizationLog(coerce string key, coerce string section, coerce string filename, out coerce string scriptResult)
{
    ...
    return Class'Engine'.static.IsEditor() || Class'SS_CommunicationSettings'.default.ToggleDebugging || scriptResult != "" && !Class'Hat_Localizer'.static.ContainsErrorString(scriptResult);
}
...
static function Array<OCSegment> CutSegments(OCSegment segment, int cutpoint1, int cutpoint2, string compare, optional bool noclose = false)
{
    local Array<OCSegment> segs;
    local OCSegment midSeg;
...
static function OCSegment CreateSegment(string text, optional Surface icon, optional Color c = MakeColor(255,255,255))
{
    local OCSegment dummy;
    dummy.Text = text;
    dummy.Icon = icon;
    dummy.Color = c;
...
// # CHAT RENDERER #
// #################
static function DrawChat(HUD H, Array<OCLogInfo> chatRender, float scale, OCSettings settings)
{
    ...
    local OCSegment currentSegment;
    local Array<OCLogInfo> chatRenderReversed;
    local bool hasImage;
...
static function Array<OCLogInfo> AdjustChatLog(HUD H, Array<OCLogInfo> chatRender, float scale, optional OCSettings settings)
{
    local int i, u, j;
    local OCSegment newSegment, currentSegment, empty;
    local Array<OCSegment> movedSegments;
    local OCLogInfo newChatLog;
...
static function OCSegment BreakSegment(HUD H, float TotalRowLength, float textSize, out OCSegment segment, float clippedLimit, optional float scale)
{
    local Array<String> words, chars;
    local string textMerge, newSegmentText, wordMerge;
    local float SegmentXLength, SegmentYLength;
    local OCSegment newSegment;
...
static function OCLogInfo CreateNewLineChatLog(OCLogInfo oldLog, Array<OCSegment> Segments)
{
    local OCLogInfo log;
    log = oldLog;
...
static function bool SegmentIsEmpty(OCSegment segment)
{
    return segment.Icon == None && Len(segment.Text) <= 0;
}

static function AddStartOfLogIndicator(out OCLogInfo loginfo, optional Hat_GhostPartyPlayerStateBase Sender = None)
{
    local OCSegment startingLine;
...
static function int GetChatOpacity(OCLogInfo log, bool forceVisiblity)
{
